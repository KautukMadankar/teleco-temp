...
if page == "📊 Overview Dashboard":
    ...
    # ── Row 4: Bundles with Highest Bundle Change ──
    st.markdown("")
    with st.spinner("Computing bundle change rates…"):
        bundle_change = (
            main_df.groupby("bundle_name")["bundle_switches_6mo"]
            .mean()
            .sort_values(ascending=False)
            .reset_index()
        )
        bundle_change.columns = ["Bundle Name", "Avg Bundle Switches (6mo)"]

    fig_bundle_change = px.bar(
        bundle_change, y="Bundle Name", x="Avg Bundle Switches (6mo)",
        orientation="h",
        title="Bundles with Highest Bundle Change (Avg Switches in 6 Months)",
        labels={"Avg Bundle Switches (6mo)": "Avg Bundle Switches (6mo)"},
        color="Avg Bundle Switches (6mo)",
        color_continuous_scale="Tealgrn",
    )
    fig_bundle_change.update_layout(**PLOTLY_LAYOUT, yaxis=dict(autorange="reversed"))
    fig_bundle_change.update_layout(coloraxis_showscale=False)
    st.plotly_chart(fig_bundle_change, use_container_width=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        # numeric_cols = numeric_cols.drop(['churn']).tolist()

        exclude_cols = ['churn', 'churn_probability_score', 'retention_offer_accepted', 'retention_offers_6mo',
                        'daily_data_limit_gb', 'avg_daily_data_used_gb', 'data_usage_ratio', 'total_data_used_gb_mo', 'data_overuse_days_mo', 'payment_delays_6mo', 'days_since_last_recharge', 'app_logins_per_week', '']
        numeric_cols = [
            col for col in filtered.select_dtypes(include="number").columns
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown('<p class="sub-header">Gemini-powered, data-driven strategy for your telecom business</p>',
                unsafe_allow_html=True)

    st.info("This page uses the **Google Gemini API**. Add your API key to continue.")
    ...
    if generate:
        if not api_key:
            ...
            with st.spinner("Analysing segment data & calling Gemini…"):
                # ── Filter segment ──
                seg = main_df.copy()
                if sel_reg != "All":
                ...
                prompt = (
                    ...
                    "recommendations, each with a bold title and 2-3 sentences of detail. Also give reason for each recommendation.\n\n"
                    f"Segment data:\n{stats_text}"
                )
                ...
                try:
                    ...
                    st.error("Please install the correct package: pip install google-genai")
...
# ══════════════════════════════════════════════════════════════
    ...
    # ── Download customers by quadrant ──
    st.markdown("---")
    st.subheader("📥 Download Customers by Quadrant")
    quadrant_names = pred_seg["segment_quadrant"].unique().tolist()
    dl_cols = st.columns(len(quadrant_names))
    for i, quad_name in enumerate(sorted(quadrant_names)):
        quad_data = pred_seg[pred_seg["segment_quadrant"] == quad_name]
        csv_quad = quad_data.to_csv(index=False).encode("utf-8")
        safe_name = quad_name.replace(" ", "_").replace("/", "_").lower()
        dl_cols[i].download_button(
            label=f"📥 {quad_name}",
            data=csv_quad,
            file_name=f"{safe_name}_customers.csv",
            mime="text/csv",
            key=f"dl_quad_{safe_name}",
        )
...
# ══════════════════════════════════════════════════════════════
    ...
    with col_ctrl1:
        bundle_threshold = st.slider("Min Bundle Switch Probability", 0.1, 0.9, 0.3, step=0.05)
    with col_ctrl2:
        conversion_rate = st.slider("Expected conversion rate (%)", 5, 50, 20, step=5) / 100
    ...
    with st.spinner("Computing upsell opportunities…"):
        ...
        if len(bundle_elig) == 0:
            ...
            # ── Additional columns ──
            arpu_delta_min = bundle_elig["arpu_delta"].min()
            arpu_delta_max = bundle_elig["arpu_delta"].max()
            bundle_elig["arpu_delta_norm"] = (bundle_elig["arpu_delta"] - arpu_delta_min) / (arpu_delta_max - arpu_delta_min + 1e-9)

            bundle_elig["priority_score"] = (
                bundle_elig["P(Bundle_Switch)"] * 0.5 +
                bundle_elig["arpu_delta_norm"] * 0.3 +
                bundle_elig["P(Company_Churn)"] * 0.2
            ).round(4)

            bundle_elig["revenue_at_risk_if_churned"] = bundle_elig["P(Company_Churn)"] * bundle_elig["clv"]

            # ── KPI Row ──
            bk1, bk2, bk3, bk4 = st.columns(4)
            bk1.metric("Upsell-Eligible Customers", f"{len(bundle_elig):,}")
            bk2.metric("Projected Monthly Upsell Revenue", format_inr(bundle_elig["arpu_delta"].sum() * conversion_rate))
            bk3.metric("Avg ARPU Delta / Customer", f"₹{bundle_elig['arpu_delta'].mean():,.0f}")
            bk4.metric("Cost of Inaction", format_inr(bundle_elig["revenue_at_risk_if_churned"].sum()))
            bk4.caption("Revenue at risk if these customers churn instead of upgrading")

            st.markdown("")

            # ── Sankey Diagram ──
            sankey_agg = (
                bundle_elig.groupby(["bundle_name", "suggested_bundle"])["arpu_delta"]
                .sum()
                .reset_index()
            )

            all_labels = list(dict.fromkeys(
                sankey_agg["bundle_name"].tolist() + sankey_agg["suggested_bundle"].tolist()
            ))
            label_to_idx = {label: i for i, label in enumerate(all_labels)}

            sankey_colors = ["#667eea", "#764ba2", "#f093fb", "#4facfe", "#43e97b",
                             "#fa709a", "#fee140", "#a18cd1"]
            node_colors = [sankey_colors[i % len(sankey_colors)] for i in range(len(all_labels))]
            ...
            fig_sankey = go.Figure(data=[go.Sankey(
                ...
                link=dict(
                    source=[label_to_idx[s] for s in sankey_agg["bundle_name"]],
                    target=[label_to_idx[t] for t in sankey_agg["suggested_bundle"]],
                    value=sankey_agg["arpu_delta"].tolist(),
                ),
            )])
            fig_sankey.update_layout(
                title="Bundle Upgrade Flow — ARPU Delta by Upgrade Path",
                height=500,
                **PLOTLY_LAYOUT,
            )
            st.plotly_chart(fig_sankey, use_container_width=True)
            st.caption("Width of each flow represents total ARPU delta opportunity across all eligible customers on that upgrade path.")

            # ── Prioritized Customer Table ──
            st.subheader("Prioritized Upsell Target List")
            ...
            display_bundle = bundle_elig[[
                ...
                arpu_col, "arpu_delta", "P(Bundle_Switch)", "P(Company_Churn)", "priority_score"
            ]].sort_values("priority_score", ascending=False).head(500)
            display_bundle = display_bundle.rename(columns={arpu_col: "Monthly ARPU ₹"})
            display_bundle = display_bundle.round(3)
            st.dataframe(display_bundle, use_container_width=True, hide_index=True, height=420)

            # ── Download full list ──
            full_export = bundle_elig.sort_values("priority_score", ascending=False)
            csv_export = full_export.to_csv(index=False).encode("utf-8")
            st.download_button(
                label="📥 Export Full Upsell Target List",
                data=csv_export,
                file_name="bundle_upsell_targets.csv",
                mime="text/csv",
            )
